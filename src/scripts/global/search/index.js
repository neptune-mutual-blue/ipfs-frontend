...
const inputClearButton = document.querySelector(
  '#ModalSearchInputSearch + button.clear'
)
...
function handleKeyDown (e) {
  if ((e.key === 'Escape' || e.which === 27) && !searchInputField.value) {
    ...
    document
      ...
      .classList.remove('no', 'vertical', 'scroll')
...
document
  .getElementById('SearchInputSearch')
  .addEventListener('focus', onActivate, { passive: true })
searchInputField.addEventListener('input', debouncedOnSearch, {
  passive: true
})
...
function updateCopyButtons () {
  const copyButtons = document.querySelectorAll(
    '.search.result .item > .footer button.copy'
  )

  const handleCopy = (e) => {
    const href = `${window.location.origin}${e.currentTarget.getAttribute(
      'data-link'
    )}`
